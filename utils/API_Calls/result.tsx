...
//     return Array;
// }
import { Id, choices } from "../UserData";

let Stage2Keys: any[] = [];

const SERVER_IP = process.env.SERVER_IP;

async function get_Stage2Key(id: string) {
    console.log(Stage2Keys);
    for (const key of Stage2Keys) {
        if (key.id === id) {
            return key;
        }
...
export async function get_result() {
    await fetchStage2Keys();
    const Array: boolean[] = [false, false, false, false];
    const my_choices = [choices.recv1, choices.recv2, choices.recv3, choices.recv4]

    const my_key = await get_Stage2Key(Id);
    if (my_key === -1) {
        console.log("Error: Key not found");
        return Array;
    }
    ...
    for (const i of [0, 1, 2, 3]) {
        const key = await get_Stage2Key(my_choices[i]);
        if (key === -1) {
            // console.log("Error: Key ", my_choices[i], " not found");
            continue;
        }
        const Ka1 = BigInt(key.Ka1);
        const Ka2 = BigInt(key.Ka2);
        const Ka3 = BigInt(key.Ka3);
        const Ka4 = BigInt(key.Ka4);
        const Kb1 = BigInt(key.Kb1);
        const Kb2 = BigInt(key.Kb2);
        const Kb3 = BigInt(key.Kb3);
        const Kb4 = BigInt(key.Kb4);
        const Kc1 = BigInt(key.Kc1);
        const Kc2 = BigInt(key.Kc2);
        const Kc3 = BigInt(key.Kc3);
        const Kc4 = BigInt(key.Kc4);
        const Kd1 = BigInt(key.Kd1);
        const Kd2 = BigInt(key.Kd2);
        const Kd3 = BigInt(key.Kd3);
        const Kd4 = BigInt(key.Kd4);

        for (const r1xr2 of [Ka1, Ka2, Ka3, Ka4, Kb1, Kb2, Kb3, Kb4, Kc1, Kc2, Kc3, Kc4, Kd1, Kd2, Kd3, Kd4]) {
            for (const myr1xr2 of [My_Ka1, My_Ka2, My_Ka3, My_Ka4, My_Kb1, My_Kb2, My_Kb3, My_Kb4, My_Kc1, My_Kc2, My_Kc3, My_Kc4, My_Kd1, My_Kd2, My_Kd3, My_Kd4]) {
                if (r1xr2 === myr1xr2) {
                    if (r1xr2 !== null || myr1xr2 !== null) {
                        Array[i] = true;
                    }
                    break;
                }
